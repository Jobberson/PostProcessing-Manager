...
/// <summary>
/// A simple runtime PostProcess manager for URP volumes.
/// Improvements: lifecycle/cancelation, safer blending, centralized keys.
/// Note: verify the VolumeComponent types exist in your URP version.
/// </summary>
public partial class PostProcessManager : Singleton<PostProcessManager>
{
    ...
    // Manage active coroutines by key (one coroutine per effect key)
    ...
    {
        if (!EnsureCached(ref colorAdjustments)) return;
    ...
    {
        if (!EnsureCached(ref bloom)) return;

        float t = Mathf.Clamp01(blendAmount);
        // Fixed: Lerp directly toward the targetIntensity (no multiply)
        float finalIntensity = Mathf.Lerp(bloom.intensity.value, targetIntensity, t);
        float finalThreshold = Mathf.Lerp(bloom.threshold.value, targetThreshold, t);

        string key = Keys.Bloom;
        StopActive(key);
        if (duration <= 0f)
        {
    ...
    {
        if (!EnsureCached(ref depthOfField)) return;
    ...
    {
        if (!EnsureCached(ref whiteBalance)) return;

        float t = Mathf.Clamp01(blendAmount);
        float finalTemp = Mathf.Lerp(whiteBalance.temperature.value, temperature, t);
        float finalTint = Mathf.Lerp(whiteBalance.tint.value, tint, t);
        string key = Keys.WhiteBalance;
        StopActive(key);
    ...
    {
        if (!EnsureCached(ref liftGammaGain)) return;

        float t = Mathf.Clamp01(blendAmount);
        Color finalLift = Color.Lerp(liftGammaGain.lift.value, lift, t);
        Color finalGamma = Color.Lerp(liftGammaGain.gamma.value, gamma, t);
        Color finalGain = Color.Lerp(liftGammaGain.gain.value, gain, t);
        string key = Keys.LiftGammaGain;
        StopActive(key);
    ...
    {
        if (!EnsureCached(ref shadowsMidtonesHighlights)) return;

        float t = Mathf.Clamp01(blendAmount);
        Color finalShadows = Color.Lerp(shadowsMidtonesHighlights.shadows.value, shadows, t);
        Color finalMidtones = Color.Lerp(shadowsMidtonesHighlights.midtones.value, midtones, t);
        Color finalHighlights = Color.Lerp(shadowsMidtonesHighlights.highlights.value, highlights, t);
        string key = Keys.SMH;
        StopActive(key);
    ...
    private bool EnsureCached<T>(ref T comp) where T : VolumeComponent
    {
        ...
        // no target or profile -> nothing to do
        if (targetVolume == null || targetVolume.profile == null) return false;

        // Try directly from the profile (fast path)
        if (targetVolume.profile.TryGet<T>(out var found) && found != null)
        {
            comp = found;
            return true;
        }

        // Fallback: attempt to repopulate all cached fields (useful if profile changed)
        CacheComponents();

        // CacheComponents() sets the field if it exists, so check again
        return comp != null;
    }
    ...
    private void CacheComponents()
    {
        ...
        // TryGet will set to null if not present — OK
    ...
    // Generic modifier (keeps original behavior)
    ...
    // Blend helper with optional unscaled time (kept simple)
    private IEnumerator BlendFloat(float start, float end, float duration, Action<float> setter, string key, bool unscaled = false)
    {
        float elapsed = 0f;
        while (elapsed < duration)
        {
    ...
    // When swapping volumes, stop existing blends to avoid cross-profile corruption
    public void RefreshProfile(Volume newVolume)
    {
        if (targetVolume == newVolume) return;
